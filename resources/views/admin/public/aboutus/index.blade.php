            ...
            <div class="table-section">
                <div
                    ...
                    <h3
                        ...
                        {{ isset($aboutUs) ? 'Edit About Us Sections' : 'Add About Us Sections' }}
                    </h3>
                    <div style="margin-bottom: 10px; color: #4F8A8B; font-size: 0.98em;">
                        ...
                        Please keep the About Us details current for members and visitors.
                    </div>
                </div>
                <form method="POST" action="{{ route('about_us.edit') }}" class="modern-form" novalidate id="aboutus-form">
                    @csrf
                    <div class="form-row">
                        <div class="form-group form-floating">
                            <input type="text" name="meta_title" id="meta_title"
                                value="{{ old('meta_title', isset($aboutUs) ? $aboutUs->meta_title : '') }}"
                                class="{{ $errors->has('meta_title') ? 'is-invalid' : (strlen(old('meta_title', isset($aboutUs) ? $aboutUs->meta_title : '')) ? 'is-valid' : '') }}"
                                placeholder="Meta Title">
                            <label for="meta_title">Meta Title</label>
                            @if($errors->has('meta_title'))
                                <div class="invalid-feedback">{{ $errors->first('meta_title') }}</div>
                            @elseif(strlen(old('meta_title', isset($aboutUs) ? $aboutUs->meta_title : '')))
                    ...
                    <div class="form-row">
                        <div class="form-group form-floating">
                            <textarea name="meta_description" id="meta_description"
                                class="{{ $errors->has('meta_description') ? 'is-invalid' : (strlen(old('meta_description', isset($aboutUs) ? $aboutUs->meta_description : '')) ? 'is-valid' : '') }}"
                                placeholder="Meta Description"
                                style="min-height: 80px;">{{ old('meta_description', isset($aboutUs) ? $aboutUs->meta_description : '') }}</textarea>
                            <label for="meta_description">Meta Description</label>
                            @if($errors->has('meta_description'))
                                <div class="invalid-feedback">{{ $errors->first('meta_description') }}</div>
                            @elseif(strlen(old('meta_description', isset($aboutUs) ? $aboutUs->meta_description : '')))
                    ...
                    <div class="form-row">
                        <div class="form-group form-floating">
                            <input type="text" name="meta_keywords" id="meta_keywords"
                                value="{{ old('meta_keywords', isset($aboutUs) ? $aboutUs->meta_keywords : '') }}"
                                class="{{ $errors->has('meta_keywords') ? 'is-invalid' : (strlen(old('meta_keywords', isset($aboutUs) ? $aboutUs->meta_keywords : '')) ? 'is-valid' : '') }}"
                                placeholder="Meta Keywords">
                            <label for="meta_keywords">Meta Keywords</label>
                            @if($errors->has('meta_keywords'))
                                <div class="invalid-feedback">{{ $errors->first('meta_keywords') }}</div>
                            @elseif(strlen(old('meta_keywords', isset($aboutUs) ? $aboutUs->meta_keywords : '')))
                    ...
                    @php
                        ...
                        $oldTitles = old('title', isset($aboutUs) && is_array($aboutUs->title) ? $aboutUs->title : []);
                        $oldPoints = old('points', isset($aboutUs) && is_array($aboutUs->points) ? $aboutUs->points : []);
                            ...
                            <div class="section-block"
                                ...
                                <div class="form-row">
                                    <div class="form-group" style="width:100%;">
                                            ...
                                            @foreach($pointsArr as $i => $point)
                                                <li class="point-row">
                                                    <input type="text" name="points[{{ $s }}][]" value="{{ $point }}"
                                                        class="point-input {{ $errors->has('points.' . $s . '.' . $i) ? 'is-invalid' : (strlen($point) ? 'is-valid' : '') }}"
                                        ...
                                        @endif
                                        @foreach($errors->get('points.' . $s . '.*') as $pointErrors)
                    ...
                    <div class="form-actions">
                        <button type="submit" class="action-button primary">
                            ...
                            {{ isset($aboutUs) ? 'Update About Us Sections' : 'Save About Us Sections' }}
                        </button>
                    </div>
                </form>
            </div>
