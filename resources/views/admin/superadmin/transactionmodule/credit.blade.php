...
@section('content')
    <main class="main-content">
                        ...
                        <div class="filter-header">
                            <div style="margin: 18px 0 8px 0;">
                                ...
                                <p style="color: #475569; font-size: 0.85em; margin-bottom: 7px;">
                                    Use the advanced filters below to quickly locate specific credit transactions by member,
                                    unique ID, date, or year.
                                </p>
                            </div>
                        </div>
                        <form method="GET" action="{{ route('transactions.credit') }}" style="width: 100%;">
                            <div style="display: flex; gap: 30px; flex-wrap: wrap; align-items: flex-end;">
                                ...
                                <div style="display: flex; flex-direction: column; min-width: 150px;">
                                    <label for="end_date"
                                        style="font-size: 0.95em; color: #64748b; margin-bottom: 0.2em;">End Date</label>
                                ...
                                <div style="display: flex; align-items: center; gap: 10px; margin-top: 1.5em;">
                                    <button type="submit"
                                        style="background: #6366f1; color: #fff; border: none; border-radius: 6px; padding: 0.5em 1.2em; font-weight: 600; height: 2.4em;">
                                        <i class="fa-light fa-filter"></i>
                                    </button>
                                    <a href="{{ route('transactions.credit') }}"
                                        style="background: #f1f5f9; color: #334155; border: none; border-radius: 6px; padding: 0.5em 1.2em; font-weight: 600; height: 2.4em; text-decoration: none;">
                                        Reset
                                    </a>
                                </div>
                            </div>
                        ...
                        <tbody>
                            @for($i = 0; $i < 6; $i++)
                                <tr>
                                    @for($j = 0; $j < 8; $j++)
                                        <td>
                                            <div style="height: 18px; width: 100%; background: linear-gradient(90deg, #f1f5f9 25%, #e2e8f0 50%, #f1f5f9 75%); background-size: 200% 100%; animation: skeleton-loading 1.2s infinite linear; border-radius: 4px;"></div>
                                        </td>
                                    @endfor
                                </tr>
                            @endfor
                        ...
                        @if($transactions->isEmpty())
                            <tr>
                                <td colspan="9" style="text-align: center; padding: 30px; color: #888;">
                                    ...
                                    <div style="margin-top: 8px;">No credit transactions found.</div>
                                </td>
                            </tr>
                        @else
                            @foreach($transactions as $transaction)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="status-badge {{ strtolower($transaction->status) == 'credit' ? 'active' : 'inactive' }}"
                                    ...
                                    <td>
                                      ...
                                      <td>
                                        @php
                                            // Determine annual fee amount for this user and year, similar to member_detail.blade.php
                                            $currentYear = now()->year;
                                            $feeYear = $selectedYear ?? null;
                                            ...
                                            // Try to get the correct annual fee setting for this user and year
     ...
     <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Correct column mapping based on table header order
            const colMap = {
                'member-id': 0,
                'name': 1,
                'amount': 2,
            ...
            function setColVisibility(col, visible) {
                ...
                if (colMap[col] !== undefined && ths[colMap[col]]) {
                    ths[colMap[col]].style.display = visible ? '' : 'none';
                }
                trs.forEach(tr => {
                    const tdsInRow = tr.querySelectorAll('td');
                    if (colMap[col] !== undefined && tdsInRow[colMap[col]]) {
                        tdsInRow[colMap[col]].style.display = visible ? '' : 'none';
                    }
                });
            }
            document.querySelectorAll('.table-controls-container input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', function () {
                    setColVisibility(cb.dataset.col, cb.checked);
                });
            });
